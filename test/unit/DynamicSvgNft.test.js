...
!developmentChains.includes(network.name)
  ...
  : describe("DynamicSvgNft Unit Tests", () => {
      const deployDynamicSvgNftFixture = async function () {
        const [deployer] = await ethers.getSigners()
        // see: https://github.com/OpenZeppelin/openzeppelin-contracts/blob/master/test/token/ERC20/ERC20.test.js#L26
        ...
        const dynamicSvgNft = await ethers.deployContract(
          "DynamicSvgNft",
          [mockV3Aggregator.target, lowSvg, highSvg],
          deployer
        )
        ...
        return {
          mockV3Aggregator,
          dynamicSvgNft,
          priceFeed: mockV3Aggregator.target,
        }
      }

      describe("Constructor", () => {
        it("sets starting values correctly", async () => {
          const { dynamicSvgNft, priceFeed: expectedPriceFeed } =
            await loadFixture(deployDynamicSvgNftFixture)
          ...
          assert.equal(priceFeed, expectedPriceFeed)
        })
      })

      describe("mintNft", () => {
        it("creates an NFT and emits an event", async () => {
          const { dynamicSvgNft } = await loadFixture(
            deployDynamicSvgNftFixture
          )
